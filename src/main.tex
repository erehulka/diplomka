...
\todo{Možno toto netreba. Keď dáme preč tie naše potential cycle separating cuts, tak to bude ok a bude stačiť vylúčiť len ten jeden bod. Asi ani nebude treba definovať, len trivial multipole = 1 bod, teda definíciu presunúť k multipólom}
\begin{definition}
	Let $M$ be a multipole. If $|M|=1$ we say that $M$ is \textit{cyclically trivial}. Otherwise it is cyclically nontrivial. 
\end{definition}
...
\begin{lemma}\label{le:mi-implies-distance}	
	Let $M$ be a connected cyclically nontrivial multipole. Let $m$ be the size of the minimal independent cut in $M$. Then the smallest distance between any pair of distinct semiedges in $M$ is at least $m-1$.
\end{lemma}
...
\begin{lemma}\label{lem:size-of-minimal-potentially-cycle-separating-after-junction}
	Let $M_1$ and $M_2$ be cyclically nontrivial multipoles, such that $m_1$ and $m_2$ are the sizes of minimal independent cuts in $M_1$ and $M_2$ respectively. Let $M$ be the result of a partial junction $M_1*M_2$ of size $j$. Then the size of the minimal independent cut in $M$ is at least $\min\{m_1,m_2,j\}$.
\end{lemma}
...
\begin{theorem}\label{th:connecting-potencially-cyclically-connected-with-number-of-resulting-semiedges}
	Let $M_1$ and $M_2$ be connected, cyclically nontrivial and potencially cyclically $k$-connected multipoles. Let $m_i$ be the size of the smallest independent cut in $M_i$. Let $M$ be a result of a partial junction $M_1*M_2$ of size $j$. Let us denote by $s_i$ the number of remaning semiedges after the partial junction in $M_i$, meaning $|S(M_i)|-j$. If $m_1+m_2\geq k$ and one of the following holds:
...
\todo{Táto lema je cool vďaka tomu, že ukazuje, že aj keď chcem mať obmedzenie na minimálny stupeň =  $k$ (aby to malo zmysel?), tak viem pre každé $k$ nájsť počet vrcholov a minimálny počet hrán medzi dvojicou vrcholov}
...
\begin{lemma}
	...
	\todo{a existuje prirodzené číslo medzi - počet vrcholov}
\end{lemma}
