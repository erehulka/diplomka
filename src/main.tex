...
\todo{articles nie na konci riadku}

\todo{cislovky cislom ked to je hodnota matematicka, slovom ked to je napriklad pocet niecoho co si viem predstavit}

\todo{Nie vsetky definicie odcitovat, ale take nejake vysledky ako ohranicenia a podobne. Alebo proste odkial su tie veci}

\todo{Definovat vsetky netrivialne veci ktore pouzivam}
...
In multipoles, as in graphs, an \textit{edge-cut} is a set of links that, when severed, disconnects the multipole. We also use edge-cuts defined as the set of edges $E(V_1, V_2)$ where $\{V_1,V_2\}$ is a partition of $V(M)$. Note that all of the edges in these edge-cut are links, since they connect two vertices.

We now define edge subdivisions in multipoles, following Gross et al. \cite{gross2018graph}.
\begin{definition}
	Let $e$ be an edge in a multipole $M$. \textit{Subdividing} the edge $e$ means that a new vertex $w$ is added to $V(G)$, and the edge $e$ is replaced according to its type:
	\begin{itemize}
...
\todo{sem napríklad tie existujúce výsledky o spájaní multipólov? Alebo proste niekde sem}
...
\todo{nedáme toto niekde do úvodu? alebo do kapitoly o cyclic parts?}
...
\begin{proof}
	...
	For $k$ equal to 1 and 2 we see that $C_k$ fulfils the requirements, we shall assume $k\geq 3$. 
	
	For a given $k$, we construct a cubic vertex-transitive graph $G$ with girth $2k$ according to \cref{th:vertex-transitive-girth-regular}. Then $\zeta(G)=2k$ by \cref{th:cyclic-connectivity-of-transitive}. Next, let $P$ be a path of length $k-3$ in $G$. Since $k\geq 3$ and $|V(G)|$ is at least $2k$ because of its girth, there will always be such a path. Sever the edges in the edge-cut ${E\left(V(P), V(G)-V(P)\right)}$ and denote the component not containing the path $P$ by $G'$. This process is depicted in \cref{fig:removing-vertices-on-path-k-3}.
...
\begin{proof}
	\todo{Nezávislý rez!!!! Tak by to šlo}
	
	\todo{Alebo skor najdeme spor a vratime sa naspat k povodnemu grafu a ukazeme ze ten cyklicky sporny multipol bude aj v povodnom. Pozor ale na to ze co ak je povodny graf? Vtedy netreba inu vetu? Mozeme len sa odvolat na lemu co mame pred tymto.}
	...
	\todo{odstránenie polhrany aj uhladenie vrchola st. 2 cykly zachová}
...
\todo{označiť miesta kde sa mení ten dôkaz?}
