...
\todo{text! Prečo je to zlé? Prečo ideme aj do tohoto?}
...
Using this we can prove a similar theorem to \cref{th:connecting-potentially-cyclically-connected-with-number-of-resulting-semiedges}:
...
\todo{text! Prečo ideme toto robiť? Nedáme to až za subdivízie, kde vysvetlíme že subdivízie vedia robiť problém lebo v nejakom momente môžeme mať menej polhrán ako k?}
...
\todo{nejak to uzavri, že máme takýto framework a čo s ním vieme robiť.}
...
\begin{example}
	Consider the graph $G_{1,i}$ presented by Máčajová and Škoviera in 2006 \cite{Macajova2006}, depicted in \todo{FIGURE}. The authors presented this graph as having cyclic connectivity equal to 6, however they have not provided a proof supporting this statement. In this example we prove this statement by utilizing our theorems.
	
	\todo{Chceme tu vysvetliť čo sú tie supervertices/superedges?}
	
	\todo{Chceme poriadne vysvetliť snarky, Isaacs snark a podobne?}
	
	\todo{Rajník, 63 Proposition 4, vie nám to pomôcť že nemá to most, ak bude treba dokazovať.}
	
	POZOR má rez 2.
	
	Postup:
	\begin{enumerate}
		\item Ukáž že tie Isaacsove komponenty sú potenciálne cyklicky k-súvislé - dá sa nahliadnuť, alebo to chceme nejak dokázať?
		\item Ukáž že najmenší nezávislý rez je aspoň 3 - rovnako, nejak dokázať, alebo proste povedať že sa dá nahliadnuť.
		\item Pospájaj
		\item Sprav subdivízie
		\item Pripoj vnútro.
	\end{enumerate}
\end{example}

\begin{example}
	Sem hento z \cite{Kochol1996}. Problém - ako sa zmení najmenší nezávislý rez po subdivíziach? POZOR je tam 2 rez
\end{example}
