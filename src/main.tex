...
An automorphism on a graph $G$ is an isomorphism from $G$ to itself, in other words a permutation of the vertices of $G$ that maps edges to edges and nonedges to nonedges. A trivial case of an automorphism is an identity permutation.
...
\todo{pridať úvod podľa feedbacku}
...
\todo{tiež podľa feedbacku upraviť}

A cyclic part minimal under inclusion is called an \textit{atom}. Two atoms of a single graph may differ in in the number of vertices \cite{atoms-of-cyclic}, those with the minimum number of vertices are called \textit{proper} atoms.
...
Several estimations regarding a lower bound of the order of these cyclic parts were proven.
...
\begin{problem}[{{\cite[Problem 2.6]{Rajnik_phd}}}]\label{problem:cyclic-part-algorithm}
...
\todo{sem sa hodí ilustrácia čo je $X_1,X_2$, mam to v tom zošite.}
