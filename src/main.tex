...
It can be noted, that the corollary of this theorem is that the cyclic $k$-poles are in this case cyclic $k$-parts, since the cyclic edge-connectivity of the graph resulting from the junction is $k$.
...
We now present a theorem which establishes the conditions under which a partial junction of two potentially cyclically $k$-connected multipoles remains potentially cyclically $k$-connected. \todo{viac textu sem určite, toto treba lepšie uviesť}
...
\todo{túto vetu asi chceme dať dopreč? Je to celkom nepekné, akože viem si to obhájiť že to je cool, ale tak to použitie je skôr take silené.}

\begin{theorem}\label{th:connecting-potentially-k-connected}
	Let $M_1$ and $M_2$ be potentially cyclically $k$-connected multipoles with the same number of semiedges, which do not contain an independent edge-cut of size $a$, such that after the cut there is an acyclic component with $b$ former semiedges (not resulting from the cut) and $b>a$. For each $M_i$, let $m_i$ denote the size of its smallest independent cut, or $\infty$ if no such cut exists. If $m_1+m_2\geq k$ then the partial junction $M=M_1*M_2$ is potentially cyclically $k$-connected unless $M$ has less than $k$ semiedges.
\end{theorem}
...
\begin{proof}
	...
	Note that there are two dangling edges from $v$. If there would be three, it would mean that in $X$ there would not be a path between this vertex and any other vertex in $X$, meaning if we did not sever the edge conncting the vertex to a vertex in $X$ \todo{blah blah blah, toto treba lepšie spísať.}
	...
	\todo{sprav asi aj ilustráciu že čo toto znamená}
\end{proof}

\begin{theorem}\label{th:subdivisions-potentially-cyclically-connected}
	Let $M$ be a potentially cyclically $k$-connected multipole. Let $M'$ be a multipole resulting from $M$ by performing a set of edge subdivisions, such that a dangling edge is added to each new edge created by the subdivision. If $|S(M')|\geq k$, then $M'$ is potentially cyclically $k$-connected.
\end{theorem}

\begin{proof}
	Suppose by contradiction that $M'$ is not potentially cyclically $k$-connected, thus contains a cyclic $l$-pole $X$ for some $l<k$. Let us denote $V(X)$ by $V_X$ and $V(M'-X)$ by $V_{M'-X}$. Since $M'$ has at least $k$ semiedges, there must be some edge-cut yielding $X$, we can denote it by $E(V_X,V_{M'-X})$.
	
	Let the cut be S.
	
	We create an edge-cut in M.
	
	For each edge in S, add to M either the edge if it is present in M, otherwise it is some $sv$ where the original edge was $wv$ and it was subdivided then add $wv$.
	
	We prove it is an edge-cut separating $V(X)\cap V(M)$ and $V(M'-X)\cap V(M)$. First we need to prove they are non-empty, thus each $V(X)$ and $V(M'-X)$ contains some vertex from $M$. $V(X)$ is easy because it is cyclic and we cannot create a cycle on vertices from subdivisions without adding some former vertices. For $V(M'-X)$ if it contained only vertices from the subdivisions, then $X$ would have at least $x_1+2x_2$ semiedges where $x_1+x_2\geq k$ so at least $k$ semiedges contradicting it is an $l$-pole. To prove it is an edge-cut suppose there is a path between $v\in V(X)\cap V(M)$ and $w\in V(M'-X)\cap V(M)$ not going through a cut edge. In $M'$ these vertices are separated by the original cut. If the edge is non-subdivided, there is an edge in the new cut, if it is there is also edge in the new cut, leading to contradiction. (we can explain it somehow).
	
	Now we prove that the component $V(X)\cap V(M)$ has less than $k$ semiedges. It can be seen that it has the same semiedges as $X$ however without those from the vertices from subdivisions, thus at most $l$.
	
	We now prove it is cyclic. This can also be shown by showing the cycle in $X$ as a path on vertices and showing equivalent path in $M$.
	
	This leads to a contradiction.
	
	\todo{} Ukáž že ten rez bude mať ekvivalent v pôvodnom grafe, mám to niekde v poznámkach.
\end{proof}

\subsection{Examples}

\todo{sem dať hlavne hentú konštrukciu 6-pólu z článku tuším od Kochola}
...
TODO: problem, superpozície. Tuším Máčajová, ale aj Kochol majú v článku graf, ktorý povedia že je cyklicky 6-súvislý, ale nemajú to nijak dokázané. Každopádne je problém že tam je ten jeden vrchol na hranách. Takže možné riešenie máme v práci, a to s tým postupným spájaním a pridávaním jednotlivých dvojíc bodov vždy keď spojíme dvojicu, a potom keď spojíme obe polky tak aj posledný bod pridáme (ale to asi nepojde).

Takže, toto chceme dať ako možný problém, konkrétne teda spomenúť, že ako vieme brať do uvahy iba také nezávislé rezy, ktoré môžu byť nejak problematické a nejak zlepšiť ten dolný odhad na veľkosť rezu keď spravíme junction?
