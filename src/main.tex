...
\todo{Možno toto netreba. Keď dáme preč tie naše potential cycle separating cuts, tak to bude ok a bude stačiť vylúčiť len ten jeden bod.}
\begin{definition}
	Let $M$ be a multipole. If $|M|=1$, or $|S(M)|\leq 1$ we say that $M$ is \textit{cyclically trivial}. Otherwise it is cyclically nontrivial. 
\end{definition}

\todo{toto chceme dať preč. Namiesto toho budeme skúmať independent cuts, keďže každý independent je potential cycle separating keď počet polhrán je aspoň 3.}
\begin{definition}
...
\todo{Berme len s aspoň troma polhranami všetko!! Potom toto je len najmenší independent cut.}
