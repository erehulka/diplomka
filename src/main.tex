...
\begin{proof}
	...
	Suppose that $V(D_i^G)-S_V\neq\emptyset$ for each $i\in \{1,2\}$, meaning there is a whole cyclic part in both $D_1$ and $D_2$. Then $G-S_v^G-S_E$ has two components $F_1^G$ and $F_2^G$ \todo{dokáž že to je rez naozaj}. The number of vertex disjoint paths from a vertex of $F_1^G$ to a vertex of $F_2^G$ in $G$ is at most $|S_V|+|S_E|\leq k-1$ \todo{prečo?}, which contradicts by Menger's Theorem that $G$ is $k$-connected.
