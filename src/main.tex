...
If there is no cycle separating edge cut in a graph $G$, we say that $\zeta(G)=\infty$ \cite{Lou2008}. \todo{Betti number!}
...
\todo{všetky tieto veci z toho článku dať preč... Nebudeme to používať.}
...
\todo{vymeniť submultipole za contains}
...
\todo{Túto vetu treba nejak modifikovať. Lebo ak ju chceme použiť aj na tie potenciálne cyklicky k-súvislé, tak nemôžeme predpokladať že počet polhrán je práve k, treba to nejak modifikovať}.
...
\todo{Toto pod tým treba len pre aspoň 2-póly, resp. ak neobsahuje taký rez daný graf tak túto hodnotu nastavme na $k$ defaultne, lebo nám to nič nepokazí asi}.
...
\todo{Definícia partial junction: NEMUSÍ spojiť všetky polhrany, ale môže (junction teda je prípad partial junction)}
...
\todo{Definovať $m_i$ rezy ako potentially cycle separating}
...
\begin{enumerate}
	...
	\item We then connect the point. Since the theorem cannot be used here, we must prove that after the connection the result will still be potencially cyclically 5-connected. \todo{prove that it will not contain the problematic cut leaving the acyclic component...}
...
\todo{we can also use \cref{th:connecting-potencially-cyclically-connected-with-number-of-resulting-semiedges} and lower the necessary conditions, this will hold as well and we won't have to prove anything about the smallest cut leaving acyclic components, the proof will be easier}

\todo{we do not have to prove that after the first junction the result will have the mininal cut of size 2, we can just use \cref{lem:size-of-minimal-potentially-cycle-separating-after-junction}. However after connecting the point we have to prove everything.}
