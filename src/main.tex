...
In our work, we allow a specific type of graph, which allows ends of its edges to not be incident with a vertex, resulting in a graph with semiedges. Such structures are called \textit{multipoles}. This term was first used by Fiol in 1991 \cite{Fiol1991}, however we will be following a definition by Nedela and Škoviera \cite{Nedela1996}.
...
A multipole is considered connected if there exists a path between any pair of edge ends. For a given multipole $M$, we define $S(M)$ to be the tuple containing all the semiedges in that multipole.
...
Let $e$ and $f$ be semiedges, such that $e\neq f$. The result of the \textit{junction} of $e$ and $f$ is a new edge between the vertices incident with $e$ and $f$ and a deletion of $e$ and $f$.
...
In multipoles, same as in graphs, an edge-cut in $M$ is a set of edges $E(V_1, V_2)$, such that $\{V_1,V_2\}$ is a partition of $V(M)$. Note that all of the edges in an edge cut are links, since they connect two vertices.

By cutting (or severing) a link $e=v_1v_2$, we mean the removal of $e$, along with adding two new semiedges: $v_1e_1$ and $v_2e_2$, where $e_1$ and $e_2$ are new edge ends, not incident with a vertex. By removing a vertex $v$ we mean the removal of all edges incident with $v$, as well as $v$ itself. \todo{not really, chceme incident spraviť na semiedge, pričom ak to už bol semiedge tak odstrániť.}
...
\begin{proof}
	\todo{Tu by som si dal pozor, lebo keď dostaneme prázdny rez tak to asi nemusí znamenať že celé to má menej ako k polhrán ale nejaký komponent či? Dať pozor na to či je to súvislé alebo nie predpokladám.}
	
	Suppose the edge-cut is not independent, meaning there exists a vertex $v$ with two edges $e_1,e_2$ in the cut. If $e_3$ is as well in the cut, we remove all three from the cut. Otherwise, modify this edge-cut by removing $e_1,e_2$ and adding the third edge $e_3$ incident with $v$ (unless $e_3$ is a semiedge, then just remove $e_1,e_2$).
...
\begin{proof}
	...
	Suppose $\delta(P')<2$, this would mean that there is a vertex in $P$ incident with more than one semiedge. Let us denote two of these semiedges by $e',\,f'$ (note that there can be three semiedges, it is however sufficient to explore two of them). In the process of junction $C_k*P$, separate junctions are performed between the semiedges $e'$ and $f'$ and some semiedges in $C_k$, say $s_1$ and $s_2$. Let $X$ be the shortest path between $s_1$ and $s_2$ in $C_k$. We see that $|X|\leq\frac{k}{2}$ since $C_k$ is a $k$-cycle. Let $e,\,f$ be the links in $G_k$ resulting from the junction of $C_k$ and $P$ such that $e$ resulted from a junction of the semiedges $e'$ and $s_1$, and $f$ from a junction of the semiedges $f'$ and $s_2$. Then there is a cycle $eXf$ in $G_k$ of length at most $\frac{k}{2}+2$. The girth of $G_k$ is $k$ thus $k\leq\frac{k}{2}+2$, leading to the fact that $k\leq4$, which leads to a contradiction. 
...
Following is a lemma that determines the upper bound on the number of vertices in an acyclic multipole. The proof uses an intuition of viewing such multipoles as forests, where instead of the leafs there are semiedges.
...
\begin{proof}
	Consider $M$ as a forest where semiedges are in place of leafs. The total edge count $e$ is thus at~most $k+|M|-1$. Counting edges by vertex degrees (where each vertex contributes three edges) and including semiedges gives us $k+3|M|$. Since this method counts each edge twice, we have:
...
\begin{proof}
	...
	\end{enumerate}
	Note that the sets of edges in the three cases are pairwise disjoint. Also, each edge between $M_1$ and $M_2$ was a semiedge in both former multipoles $M_1$ and $M_2$. Now by the definition of $d_1$ and $d_2$ we obtain:
...
\begin{proof}
	...
	We prove that the distance between any pair of semiedges in $G'$ is at least $k-2$ by contradiction. Suppose that there are two semiedges $s_1$ and $s_2$, such that ${d(s_1,s_2)\leq k-3}$. Let us denote the shortest path between them as $Q$. Before severing the edges, these semiedges were formerly edges incident with vertices $v_1$ and $v_2$ in $P$ respectively, not necessarily distinct. Since $v_1$ and $v_2$ are on $P$, it holds that $d(v_1,v_2)\leq k-3$, let us denote the shortest path as $P'$. Now let $e_1$ and $e_2$ be the edges which, after the severing the edges, became semiedges $s_1$ and $s_2$ respectively. Then $e_1Qe_2P'$ is a closed path of length at most $2+(k-3)+(k-3)=2k-4$, contradicting the fact that $G$ has girth $2k$.
...
\todo{sem sa hodí ilustrácia čo je $X_1,X_2$}
...
\begin{example}
	...
	The author presented this graph as having cyclic connectivity equal to 6, however without a formal proof. In this example we prove this statement by utilizing our theorems.
		...
		\caption{Partial junction $A_1*A_2$}
		\label{fig:Kochol-first-partial-junction}
	\end{figure}
	
	First observation is that $A$ is potentially cyclically 6-connected. It has six semiedges, and we can verify that it does not contain a cyclic multipole with at most five semiedges. Secondly we see that the size of a smallest independent edge-cut in $A$ is two. However, when performing a partial junction between two copies of $A$ ($A_1*A_2$ as shown in \cref{fig:Kochol-first-partial-junction}), by denoting the semiedges used in the partial junction in $A_1$ and $A_2$ by $X_1$ and $X_2$ respectively, we can verify that $I_S(A_i,X_i)\geq 3$ for both $i\in\{1,2\}$. By \cref{th:potentially-cyclically-connected-independent-semiedge-index} the result of the partial junction $A_1*A_2$ depicted in \cref{fig:Kochol-first-partial-junction} is potentially cyclically 6-connected. Let us denote this construction by $M_1$. We can verify that $M_1$ has no independent 2-edge-cut, thus $i(M_1)\geq 3$, which implies $I_S(M_1, X)\geq 3$ for any $X\subseteq S(M_1)$.
	...
	Next, we perform several edge subdivisions to obtain the multipole $G'$ depicted in \cref{fig:Kochol-after-subdivisions}. By \cref{th:subdivisions-potentially-cyclically-connected}, $G'$ is potentially cyclically 6-connected, and by \cref{lem:subdivisions-smallest-independent-cut}, $i(G')\geq 6$.
