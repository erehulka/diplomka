...
An Efficient Algorithm for Cyclic Edge Connectivity of Planar Graphs \cite{Lu2009}

An Algorithm for Cyclic Edge Connectivity of Cubic Graphs \cite{Dvorak2004}

Also maybe \cite{Lou2005}

\chapter{Junctions of cyclic multipoles}

\begin{definition}
	Let $M$ be a multipole. We say that $M$ is \textit{potentially cyclically $k$-connected} if it has at least $k$ semiedges when not a graph, and there exists no independent edge-cut whose removal yields a cyclic component with less than $k$ semiedges. \todo{aj keď je acyklický či?}
\end{definition}

\begin{lemma}\label{lem:graphs-potential-and-normal-cyclic-connected-eq}
	A graph $G$ is cyclically $k$-connected for integer $k$ if and only if it is potentially cyclically $k$-connected.
\end{lemma}

\begin{proof}
	Suppose $G$ is cyclically $k$-connected but not potentially cyclically $k$-connected. Then there exists an independent edge-cut yielding a cyclic component with $l<k$ semiedges. Since the edge-cut is independent, the second component must also be cyclic with $l$ semiedges, contradicting the fact that $G$ is cyclically $k$-connected.
	
	Conversely, if $G$ is potentially cyclically $k$-connected but not cyclically $k$-connected, then there exists an independent cycle-separating edge-cut of size $l<k$, separating $G$ into cyclic $l$-poles $M_1$ and $M_2$. However this directly contradicts that it is potentially cyclically $k$-connected.
\end{proof}
...
We now present a lemma that determines the precise number of vertices in a connected acyclic multipole. The proof uses an intuition of viewing such multipoles as trees, where instead of the leaf vertices there are semiedges.

\begin{lemma}\label{lem:number-of-vertices-in-acyclic-k-pole}
	Let $M$ be a connected acyclic $k$-pole for some integer $k$. Then $|M|= k-2$. 
\end{lemma}

\begin{proof}
	Consider $M$ as a tree where semiedges are in place of leaf vertices. The total edge count is thus $k+|M|-1$. Counting edges by vertex degrees (where each vertex contributes three edges) and including dangling edges gives us $k+3|M|$. Since this method counts each edge twice, we have:
	$$k+3|M|=2k+2|M|-2.$$
	Solving this equation yields $|M|=k-2$.
\end{proof}
...
\begin{proof}
	...
	\begin{figure}
		\centering
		\begin{tikzpicture}[scale=2,every node/.style={fill=none}]
			% Define coordinates
			...
			\draw[line width=5pt,gray!60] (s1) -- node[shift={(-0.4,0.8)},black] {$b$} (s3);
			\draw[line width=5pt,gray!60] (s1) -- node[above left,black] {$a$} (s2);
			\draw[line width=5pt,gray!60] (s2) -- node[shift={(0.4,0.8)},black] {$d$} (s4);
...
\todo{sem corollary, že ak máme súčet minimálnych rezov aspoň k, tak zabezpečíme obvod - pôvodné ale musia byť potenciálne cyklicky k-súvislé.}
...
\begin{proof}
	...
	\begin{figure}
		\centering
		\begin{tikzpicture}[every node/.style={circle, draw, fill=black, minimum size=7pt, inner sep=0pt}]
			\draw[rounded corners] (0,-1) rectangle (2,0);
			\node (A) at (1,0.7) {};
			\draw (A) -- ++(0,-0.7);
			\draw (A) -- ++(-0.5,-0.7);
			\draw (A) -- ++(0.5,-0.7);
			
			\node[fill=none, draw=none] (rightarrow) at (3,-0.2) {$\Rightarrow$};
			
			\draw[rounded corners] (4,-1) rectangle (6,0);
			\draw (5,0) -- ++(0,0.7);
			\draw (4.5,0) -- ++(0,0.7);
			\draw (5.5,0) -- ++(0,0.7);
		\end{tikzpicture}
		\caption{Creating a 3-pole by removing one vertex}
		\label{fig:creating-3-pole-removing-1-vertex}
	\end{figure}
...
\begin{lemma}\label{le:mi-implies-distance}	
	Let $M$ be a connected multipole and let $m$ be an arbitrary integer greater than zero. Then the smallest distance between any pair of distinct semiedges in $M$ is at least $m-1$ if it holds that $m\leq 3$ or $g(M)\geq m$ and the size of the minimal independent edge-cut in $M$ is at least $m$.
\end{lemma}

\begin{proof}
	\todo{fixnúť dôkaz - mám to v poznámkach}
...
\begin{lemma}\label{lem:size-of-minimal-potentially-cycle-separating-after-junction}
	Let $M_1$ and $M_2$ be multipoles, such that $m_1$ and $m_2$ are the sizes of minimal independent cuts in $M_1$ and $M_2$ respectively, or $\infty$ if no such cut exists in given multipole. Let $M$ be the result of a partial junction $M_1*M_2$ of size $j$. Then the size of the minimal independent cut in $M$ is at least $\min\{m_1,m_2,j\}$.
\end{lemma}

\begin{proof}
	Suppose that the minimal independent cut in $M$ has size $m_X$ and it holds that $m_X<\min\{m_1,m_2,j\}$.
	
	Since $m_X<j$, by cutting only edges which were created by the partial junction we do not obtain a disconnected graph. This means, that the cut needs to contain some edges from $M_1$ or $M_2$, suppose $M_1$. However, since these edges would be an independent cut in $M_1$, it would mean that $m_X\geq m_1$, leading to a contradiction.
\end{proof}

\begin{theorem}\label{th:connecting-potentially-cyclically-connected-with-number-of-resulting-semiedges}
	Let $M_1$ and $M_2$ be connected potentially cyclically $k$-connected multipoles. Let $m_i$ be the size of the smallest independent cut in $M_i$, or $\infty$ if no such cut exists. Let $M$ be a result of a partial junction $M_1*M_2$ of size $j$. Let us denote by $s_i$ the number of remaning semiedges after the partial junction in $M_i$, meaning $|S(M_i)|-j$. If $m_1+m_2\geq k, s_1+s_2\geq k, m_1+s_2\geq k$ and $s_1+m_2\geq k$, then $M$ is potentially cyclically $k$-connected.
\end{theorem}
...
\begin{proof}
	...
	The cut $T$ separating $X_1$ from $X_2$ is independent. Now we shall explore $A_3$ and $A_4$. If both are non-empty, $c$ and $f$ represent independent cuts in $M_1$ and $M_2$ respectively, meaning $c\geq m_1$ and $f\geq m_2$. However $m_1+m_2\geq k$, thus $c+f\geq k$, contradicting the assumption that $b+c+d+f+x_1+x_2<k$.
	
	Now suppose one of $A_3$ and $A_4$ is empty, say $A_3$. In this case $c$ represents an independent edge-cut in $M_1$, thus $c\geq m_1$. Also we see that in this case $x_3=0$ and thus $x_2=s_2$. Since $m_1+s_2\geq k$, it holds that $c+x_2\geq k$, contradicting the assumption that $b+c+d+f+x_1+x_2<k$.

	Since everything leads to a contradiction, $M$ is potentially cyclically $k$-connected.
\end{proof}

\begin{theorem}\label{th:cyclic-edge-connectivity-of-potentially-with-independent-cuts}
	Let $M_1$ and $M_2$ be connected potentially cyclically $k$-connected multipoles. Let $m_i$ be the size of the smallest independent cut in $M_i$, or $\infty$ if no such cut exists in $M_i$. Let $G$ be a junction $M_1*M_2$. If $m_1+m_2\geq k$ then $G$ is cyclically $k$-connected.
\end{theorem}

\begin{proof}
	Let us denote the cut obtained after the junction $M_1*M_2$ by $S$. Suppose by contradiction that $G$ contains a cycle separating edge cut $T$ of minimal size, such that $|T|=l<k$ and it separates $G$ into two components $X_1$ and $X_2$. Note that in this case $\zeta(G)=l$. Let $A_1,A_2,A_3,A_4$ denote the subgraphs of $G$ induced by $V(X_1)\cap V(M_1), V(X_1)\cap V(M_2), V(X_2)\cap V(M_2), V(X_2)\cap V(M_1)$ respectively. Let $a,b,c,d,e$ and $f$ denote the number of edges connecting $A_1$ and $A_2$, $A_1$ and $A_3$, $A_1$ and $A_4$, $A_2$ and $A_4$, $A_3$ and $A_4$, $A_2$ and $A_3$ respectively as seen in \cref{fig:rajnik-cyclic-part-junction-illustration}.
	
	We first prove that $A_1$ and $A_2$ are non-empty. Suppose that one is empty, say $A_1$. Then $M_2$ would contain the whole cyclic $l$-pole $X_1$, contradicting the assumption about $M_2$.
	
	Next we see that $A_3$ and $A_4$ cannot be empty at the same time, otherwise $X_2$ would be empty, contradicting that $T$ is an edge-cut.
	
	The cut $T$ separating $X_1$ from $X_2$ is independent. Now we shall explore $A_3$ and $A_4$. If both are non-empty, $c$ and $f$ represent independent cuts in $M_1$ and $M_2$ respectively. If at least one of $M_1$ and $M_2$ does not contain an independent edge-cut, this directly leads to a contradiction. Otherwise this means that $c\geq m_1$ and $f\geq m_2$. However $m_1+m_2\geq k$, thus $c+f\geq k$, contradicting the assumption that $b+c+d+f+x_1+x_2<k$.
	
	Now suppose one of $A_3$ and $A_4$ is empty, say $A_3$. Then $A_4$ is cyclic, since each minimal independent edge-cut in cubic graphs yields two cyclic components. However this would mean that $A_4$ is a cyclic $l$-pole in $M_1$, leading to a contradiction.
	
	Since everything leads to a contradiction, $G$ is cyclically $k$-connected.
\end{proof}

\begin{theorem}\label{th:connecting-potentially-k-connected}
	Let $M_1$ and $M_2$ be connected and potentially cyclically $k$-connected multipoles with the same number of semiedges, which do not contain an independent edge cut of size $a$, such that after the cut there is an acyclic component with $b$ former semiedges (not resulting from the cut) and $b>a$. Let $m_1$ and $m_2$ be the sizes of minimal independent cuts in $M_1$ and $M_2$ respectively, or $\infty$ if no such cut exists. If $m_1+m_2\geq k$ then the partial junction $M=M_1*M_2$ is potentially cyclically $k$-connected unless $M$ has less than $k$ semiedges.
\end{theorem}

\begin{proof}
	Let us denote the $i$-cut obtained after the partial junction junction $M_1*M_2$ by $S$. Suppose by contradiction that $M$ contains a cyclic $l$-pole $X_1$ such that $l<k$, $l$ is the smallest possible and $X_1$ is the largest of such $l$-poles contained in $M$ by the number of vertices. Let $T$ be the minimal edge-cut in $M$ yielding $X_1$, if such cut exists, and let the second component be $X_2$. Let $A_1,A_2,A_3,A_4$ denote the subgraphs of $G$ induced by $V(X_1)\cap V(M_1), V(X_1)\cap V(M_2), V(X_2)\cap V(M_2), V(X_2)\cap V(M_1)$ respectively. Let $a,b,c,d,e$ and $f$ denote the number of edges connecting $A_1$ and $A_2$, $A_1$ and $A_3$, $A_1$ and $A_4$, $A_2$ and $A_4$, $A_3$ and $A_4$, $A_2$ and $A_3$ respectively. Let $x_i$ be the number of dangling edges from $A_i$ as seen in \cref{fig:illustration-for-multiple-multipoles-proof}. By looking at the illustration we see that $b+c+d+f+x_1+x_2=l<k$.
	
	We first prove that $M_2$ contains a cycle. It must be non-empty, otherwise $M=X_1$ and $M$ would have less than $k$ semiedges, leading to a contradiction. Now, suppose that $M_2$ does not contain a cycle, thus is a forest. Let's look at any tree in this forest, $T$. The tree contains $a$ semiedges resulting from the cut and $b$ former semiedges. If $b\leq a$ it would lead to a contradiction that $X_1$ has the least semiedges and is the largest possible, by adding this tree to $X_1$ we would obtain a cyclic multipole with at most semiedges as $X_1$ and larger. Thus $b>a$. 
...
\begin{example}\label{ex:junctions-multiple-first}
	...
	Let $M_1,M_2,M_3$ be potentially cyclically 5-connected 5-poles, with sizes of their minimal independent cuts $m_1,m_2,m_3$ respectively, such that $m_1\geq 2,m_2\geq 3$ and $m_3\geq 3$.
	...
	Now we perform a partial junction with the singular point and $M_{12}$ resulting in $M'$ as shown in \cref{fig:3-5-poles-connected-step-2}. Since it is trivial, we can not utilize any of the theorems proved so far. Because of this, we have to prove that the result will be potentially cyclically 5-connected.
...
\todo{Metódy ako vytvoriť potenciálne cyklicky $k$-súvislý multipól. Napr. odstránením vrcholov na ceste, odstránením vrcholov mimo cesty a pod.}

\todo{Pre každú metódu nájsť výhodu, napr. vzdialenosť medzi polhranami, alebo minimálny nezávislý rez výsledku}
...
\begin{problem}
	Consider a potentially cyclically $k$-connected multipole $M$ and a multipole $K_1$ consisting of a single vertex with three dangling edges. What conditions are necessary for their partial junction $M* K_1$ to be potentially cyclically $k$-connected?
\end{problem}
